...
public class ArrayUtils {
	...
	/**
	 * Returns a flag indicating whether the container list contains an array
	 * which matches the specified match array.
	 *
	 * @param match
	 *            the array to match
	 * @param container
	 *            the list of arrays to test
	 ...
	 *
	 * @param coordinates
	 *            an array of integer coordinates
	 * @return a flat index
	 */
	 ...
	 *
	 * @param arr
	 *            an binary array (0's and 1's only)
	 * @return
	 */
	 ...
	 * each of the argument sequences. The returned list is truncated in length
	 * to the length of the shortest argument sequence.
	 *
	 * @param arg1
	 *            the first list to be the zero'th entry in the returned tuple
	 * @param arg2
	 *            the first list to be the one'th entry in the returned tuple
	...
	/**
	 * Returns an array with the same shape and the contents converted to
	 * doubles.
	 *
	 * @param ints
	 *            an array of ints.
	...
	/**
	 * Performs a modulus on every index of the first argument using the second
	 * argument and places the result in the same index of the first argument.
	 *
	 * @param a
	 * @param b
	 * @return
	...
	/**
	 * Returns a double array whose values are the maximum of the value in the
	 * array and the max value argument.
	 *
	 * @param doubs
	 * @param maxValue
	 * @return
	...
	/**
	 * Returns an array of identical shape containing the maximum of the values
	 * between each corresponding index. Input arrays must be the same length.
	 *
	 * @param arr1
	 * @param arr2
	 * @return
	...
	/**
	 * Returns an array of identical shape containing the minimum of the values
	 * between each corresponding index. Input arrays must be the same length.
	 *
	 * @param arr1
	 * @param arr2
	...
	/**
	 * Returns an array of values that test true for all of the specified
	 * {@link Condition}s.
	 *
	 * @param values
	 * @param conditions
	 ...
	 * @return
	 */
	public static double[] retainLogicalAnd(double[] values,
	 ...
	 *             if the two argument arrays are not the same length
	 */
	public static double[] divide(double[] dividend, double[] divisor,
		...
		for (int i = 0; i < dividend.length; i++) {
			quotient[i] = (dividend[i] + dividendAdjustment)
					/ ((denom = divisor[i] + divisorAdjustment) == 0 ? 1
							: denom); // Protect against division by 0
		}
		return quotient;
	}
	...
	public static double[] divide(int[] dividend, int[] divisor) {
		...
		for (int i = 0; i < dividend.length; i++) {
			quotient[i] = (dividend[i])
					/ (double) ((denom = divisor[i]) == 0 ? 1 : denom); // Protect
																		// against
																		// division
																		// by 0
		}
	...
	public static double[] divide(double[] dividend, double divisor) {
		...
		for (int i = 0; i < dividend.length; i++) {
			quotient[i] = (dividend[i])
					/ (double) ((denom = divisor) == 0 ? 1 : denom); // Protect
																		// against
																		// division
																		// by 0
		}
	 ...
	 *             if the two argument arrays are not the same length
	 */
	public static double[] roundDivide(double[] dividend, double[] divisor,
			int scale) {
		...
		double[] quotient = new double[dividend.length];
		for (int i = 0; i < dividend.length; i++) {
			quotient[i] = (dividend[i]) / (divisor[i] == 0 ? 1 : divisor[i]); // Protect
																				// against
																				// division
																				// by
																				// 0
			quotient[i] = new BigDecimal(quotient[i]).round(
					new MathContext(scale, RoundingMode.HALF_UP)).doubleValue();
		}
		return quotient;
	}
	...
	public static double[] multiply(double[] multiplicand, double[] factor,
		...
		for (int i = 0; i < multiplicand.length; i++) {
			product[i] = (multiplicand[i] + multiplicandAdjustment)
					* (factor[i] + factorAdjustment);
		}
		return product;
	}
	...
	/**
	 * Returns a new array containing the result of multiplying each index of
	 * the specified array by the 2nd parameter.
	 *
	 * @param array
	 * @param d
	 * @return
	...
	/**
	 * Returns an integer array containing the result of subtraction operations
	 * between corresponding indexes of the specified arrays.
	 *
	 * @param minuend
	 * @param subtrahend
	 * @return
	...
	/**
	 * Subtracts the contents of the first argument from the last argument's
	 * list.
	 *
	 ...
	 * @return
	 */
	public static List<Integer> subtract(List<Integer> subtrahend,
			List<Integer> minuend) {
	...
	/**
	 * Returns the passed in array with every value being altered by the
	 * addition of the specified amount.
	 *
	 * @param arr
	 * @param amount
	 * @return
	...
	/**
	 * Returns the passed in array with every value being altered by the
	 * addition of the specified double amount at the same index
	 *
	 * @param arr
	 * @param amount
	 * @return
	...
	/**
	 * Returns the passed in array with every value being altered by the
	 * addition of the specified double amount
	 *
	 * @param arr
	 * @param amount
	 * @return
	...
	/**
	 * Sparse or due to the arrays containing the indexes of "on bits", the
	 * <em>or</em> of which is equal to the mere combination of the two
	...
	/**
	 * Returns an array which starts from lowerBounds (inclusive) and ends at
	 * the upperBounds (exclusive).
	 *
	 * @param lowerBounds
	 *            the starting value
	 * @param upperBounds
	 ...
	 * @param interval
	 *            the amount by which to increment the values
	 * @return
	 */
	public static double[] arange(double lowerBounds, double upperBounds,
			double interval) {
	 ...
	 *
	 * @param nums
	 *            an unsorted array of integers with possible duplicates.
	 * @return
	 */
	...
	/**
	 * Called to merge a list of dimension arrays into a sequential row-major
	 * indexed list of coordinates.
	 *
	 * @param dimensions
	 *            a list of dimension arrays, each array being a dimension of an
	 *            n-dimensional array.
	 ...
	 *
	 * @param values
	 *            the values to alter if at the specified indexes.
	 * @param indexes
	 *            the indexes of the values array to alter
	 * @param setTo
	 *            the value to set at the specified indexes.
	 */
	...
	/**
	 * Sets the values in range start to stop to the value specified. If stop <
	 * 0, then stop indicates the number of places counting from the length of
	 * "values" back.
	 *
	 * @param values
	 *            the array to alter
	 ...
	 * @param setTo
	 *            the value to set the indexes to
	 */
	...
	/**
	 * Returns a random, sorted, and unique array of the specified sample size
	 * of selections from the specified list of choices.
	 *
	 * @param sampleSize
	 *            the number of selections in the returned sample
	 * @param choices
	 ...
	 */
	public static int[] sample(int sampleSize, TIntArrayList choices,
			Random random) {
	...
	/**
	 * Ensures that each entry in the specified array has a min value equal to
	 * or greater than the specified min and a maximum value less than or equal
	...
	/**
	 * Ensures that each entry in the specified array has a min value equal to
	 * or greater than the min at the specified index and a maximum value less
	...
	/**
	 * Returns the count of values in the specified array that are greater than
	 * the specified compare value
	 *
	 * @param compare
	 ...
	 * @return the count of values greater
	 */
	public static int valueGreaterCountAtIndex(double compare, double[] array,
			int[] indexes) {
		int count = 0;
		for (int i = 0; i < indexes.length; i++) {
	 ...
	 *
	 * @param amount
	 *            the amount to raise the values
	 * @param values
	 *            the values to raise
	 */
	 ...
	 *            the values to test
	 * @param c
	 *            the condition used to test each value
	 * @return
	 */
	...
	/**
	 * Makes all values in the specified array which are less than or equal to
	 * the specified "x" value, equal to the specified "y".
	 *
	 * @param array
	 * @param x
	 *            the comparison
	 * @param y
	 *            the value to set if the comparison fails
	 */
	public static void lessThanOrEqualXThanSetToY(double[] array, double x,
			double y) {
		for (int i = 0; i < array.length; i++) {
			if (array[i] <= x)
				array[i] = y;
	...
	/**
	 * Makes all values in the specified array which are greater than or equal
	 * to the specified "x" value, equal to the specified "y".
	 *
	 * @param array
	 * @param x
	 *            the comparison
	 ...
	 *            the value to set if the comparison fails
	 */
	public static void greaterThanOrEqualXThanSetToY(double[] array, double x,
			double y) {
		for (int i = 0; i < array.length; i++) {
			if (array[i] >= x)
				array[i] = y;
	...
	/**
	 * Makes all values in the specified array which are greater than the
	 * specified "x" value, equal to the specified "y".
	 *
	 * @param array
	 * @param x
	 *            the comparison
	 ...
	 *
	 * @param array
	 *            the array to find the max value index in
	...
	/**
	 * Returns a new array containing the items specified from the source array
	 * by the indexes specified.
	 *
	 * @param source
	 * @param indexes
	 * @return
	...
	/**
	 * Returns a new 2D array containing the items specified from the source
	 * array by the indexes specified.
	 *
	 * @param source
	 * @param indexes
	 * @return
	...
	/**
	 * Returns a new int array containing the or'd on bits of both arg1 and
	 * arg2.
	 *
	 * @param arg1
	 * @param arg2
	...
	/**
	 * Copies the passed array <tt>original</tt> into a new array except first
	 * element and returns it
	 *
	 * @param original
	 *            the array from which a tail is taken
	...
	/**
	 * Set <tt></tt>value for <tt>array</tt> at specified position
	 * <tt>indexes</tt>
	 *
	 * @param array
	 * @param value
	...
	/**
	 * Assigns the specified int value to each element of the specified any
	...
	/**
	 * Return True if all elements of the <tt>values</tt> have evaluated to true
	 * with <tt>condition</tt>
	 *
	 * @param values
	 * @param condition
	 * @param <T>
	 * @return
	 */
	public static <T> boolean all(final int[] values,
